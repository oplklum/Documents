	...
	var profileMenudWHeading = "		<li class=\"ibm-dropdown-menu-primary menu-heading\" role=\"presentation\">" +
		"				<span tabindex=\"0\">developerWorks</span>" +
		"			</li>";
	var profileMenudW = "			<li class=\"ibm-dropdown-menu-primary\" role=\"presentation\">" +
		"				<a href=\"" + dwProfileMainURL + "\" tabindex=\"-1\">" +
		"					<span>" + dwProfileMain + "</span>" +
		"				</a>" +
		"			</li>" +
...
*/
	...
	IBMCore.common.module.masthead.subscribe("profileMenuReady", "customjs", function() {
		// console.log("masthead profileMenuReady");
				...
				if(data.status == "true") {
					...
					if (showScreenName) {
						IBMCore.common.module.masthead.showProfileLinkSignedin({
							...
							linkAlt: "Signed in as " + data.screenname, // This is the text a screen reader will read aloud.
						});
					...
					//Call WS to get IBMid profile data
					IBMCore.common.util.coreservices.makeRequest("172", "ibmdwUserCallback", {}, true); 
	...
	function showUserInfo(data) {
		...
		var signedinData = {
			linkContents: "",
			linkAlt: "Signed in as " + screenname,
			backgroundImage: "data:image/png;base64," + data.Base64Image,
			type: "image"
		};

		// Show the profile link as "signed in"
		IBMCore.common.module.masthead.showProfileLinkSignedin(signedinData);
				
		if (data.UnreadCount > 0) {			
			IBMCore.common.module.masthead.showNotificationCount(data.UnreadCount);
			$("li[data-linktype='notifications'] a span").addClass("ibm-notifications-alt");
			$("li[data-linktype='notifications'] a span").text(data.UnreadCount);
		}
	}
	window.ibmdwUserCallback = showUserInfo;
