...
var optionalVcrBox = {
	...
	addOptionalVcrSection : function (place, afterElemId){
		...
		if(this.vcrOptionalList!=null && this.vcrOptionalList.length>0){
			...
			if(place == "top" && ($('.f_sponsorbox').length > 0 ||
					...
					$('.f_sponsorbox_top_2').length > 0 ||
					$('.f_sponsorbox_bottom_1').length > 0 ||
					$('.f_sponsorbox_bottom_2').length > 0)) {
				boxClassName += "bottom";
			} else {
				boxClassName += place;
			}
			...
			var vcrClassName = "f_vcrrightbar_optional_"+place;
			if(this.abTestVersion=="A"){
				vcrClassName += "_2";
			} else {
				vcrClassName += "_1";
			...
			if(this.vcrOptionalList.length>2){
				...
				$( "#optionalVcr_ul_"+place ).append(this.addWidgetVcrItem(this.vcrOptionalList[index1],vcrClassName));
				$( "#optionalVcr_ul_"+place ).append(this.addWidgetVcrItem(this.vcrOptionalList[index2],vcrClassName));
			}else{
				$.each(this.vcrOptionalList, function(i,vcrItem) {
					//f_vcrrightbar_optional is used for impression tracking
					$( "#optionalVcr_ul_"+place ).append(optionalVcrBox.addWidgetVcrItem(vcrItem,vcrClassName));
				});
			}
	...
	addVcrGATracking: function(){		
		return "&utm_source=infoq&utm_medium=related_content_native_link&utm_campaign=relatedContent_native_"+contentUriMapping+"_clk";
	},
