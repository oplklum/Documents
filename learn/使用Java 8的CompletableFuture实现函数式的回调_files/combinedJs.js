	...
	processResultForMarkAsRead : function(data){
		...
		if(data.code == 0){
			...
			if(data.message =="all") {
				...
				// set the notification counter
				if(notifCount>1){
					$(".notificationWidget__circle .circle-inner").text(notifCount-1);
				}else{
					$(".notificationWidget__circle .circle-inner").text("");
					$(".notificationWidget__circle").hide();
				}
		...
		$("."+trackingClass).each(
			function doTrack(index) {
				...
				if(vcrValues[4] && vcrValues[3]) {
					impression_location = "RIGHTBAR_BOX_TYPE" + vcrValues[4] + "_";
					if(vcrValues[3] == "top") {
						impression_location += "TOP";
					} else if(vcrValues[3] == "bottom"){
						impression_location += "BOTTOM";
					}
				} else {
		...
		$("."+trackingClass).each(
			function doTrack(index) {
				...
				if(vcrValues[3] && vcrValues[2]) {
					impression_location = "RIGHTBAR_BOX_TYPE" + vcrValues[3] + "_";
					if(vcrValues[2] == "top") {
						impression_location += "TOP";
					} else if(vcrValues[2] == "bottom"){
						impression_location += "BOTTOM";
					}
				} else {
					impression_location = "LOCATION_UNAVAILABLE";
...
var UserActions_Personalization = {
    followTopic: function (isFollowing, id, labelUserOrTopic) {
        UserActions_Personalization.requestFollow(isFollowing, id, labelUserOrTopic);
    },
    ...
    followUser: function (isFollowing, id, labelUserOrTopic) {
        UserActions_Personalization.requestFollow(isFollowing, id, labelUserOrTopic);
        if(!isFollowing) {
    ...
    followSmall: function (followVarName, id, target, f) {
        UserActions_Personalization.requestFollowSmall(followVarName, id, '#'+target+f+id, target);
    },
    requestFollow: function (isFollowing, id, labelUserOrTopic) {
        if (!processing) {
            actionName = labelUserOrTopic === '' ? 'User' : 'Topic';
            processing = true;
            $('#toggleFollow').prop('disabled', true);
            $.ajax({
                type: "GET",
    ...
    requestFollowSmall: function (followVarName, id, btnId, target) {
        if (!processing) {
            processing = true;
    ...
    setupFollowBig: function (toggle, labelUserOrTopic) { /*TODO P13N the "follow" functions can be refactored to one*/
    if(typeof btnForceHide === "undefined" || !btnForceHide) {
        $("#toggleFollow").css('display', showFollowAndLike);
        if (loggedIn) {
            if (toggle) {
                isFollowed = !isFollowed;
            }
            $('#toggleFollow').removeClass();
            if (isFollowed) {
                $('#toggleFollow').addClass('user-button-grey').html('<span class="icon-remove_circle_outline"></span>' + followBtnLabels[1] + ' ' + labelUserOrTopic);
            } else {
                $('#toggleFollow').addClass('user-button-blue').html('<span class="icon-add_circle_outline"></span>' + followBtnLabels[0] + ' ' + labelUserOrTopic);
            }
        }
    }
    },
