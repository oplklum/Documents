...
function getVcrAnchorUrl(vcr, uriMapping) {
	var vcrAnchorUrl = "";
	...
	if(uriMapping != undefined && uriMapping != null && uriMapping != "") {
		vcrAnchorUrl += '&utm_source=' + source + '&utm_medium=VCR&utm_campaign=vcr_' + uriMapping + '_click';
	} else {
		vcrAnchorUrl += '&utm_source=' + source + '&utm_medium=VCR&utm_campaign=vcr_homePage_click';
	}

	return vcrAnchorUrl;
}

function getOptionalVcrAnchorUrl(vcr, uriMapping) {
    var vcrAnchorUrl = "";
    vcrAnchorUrl += InfoQConstants.countryCode + '/vendorcontent/show.action?vcr=' + vcr.id;
    if(uriMapping != undefined && uriMapping != null && uriMapping != "") {
        vcrAnchorUrl += '&utm_source=infoq&utm_medium=related_content_native_link&utm_campaign=relatedContent_native_' + uriMapping + '_clk';
    } else {
        vcrAnchorUrl += '&utm_source=infoq&utm_medium=related_content_native_link&utm_campaign=relatedContent_native_homePage_clk';
    }
...
function getCommonElements(vcrs, uriMapping, place) {
	...
	for(var vcr in vcrs) {
		...
		vcrAnchor.attr("href", getVcrAnchorUrl(vcrs[vcr], uriMapping));
...
function getCommonMobileElement(vcr, uriMapping) {
	...
	anchor.attr("href", getVcrAnchorUrl(vcr, uriMapping));
...
function getABC2Elements(standaloneVcr, uriMapping, $firstBox, $secondBox) {
	...
	vcrCoverMain.attr("href", getVcrAnchorUrl(standaloneVcr, uriMapping));
...
function getPopUpVcr(vcr, uriMapping) {
	...
	popUpVcr.attr("href", getVcrAnchorUrl(vcr, uriMapping));
...
function getOptionalElements(vcrs, params){
	for(var vcr in vcrs) {
		...
		anchor.attr("href", getOptionalVcrAnchorUrl(vcrs[vcr], params.uriMapping));
...
function toggleAds(vcrs, params) {
	...
	setTimeout(function () {
		...
		var ifr1_0=theDiv.find("iframe:nth-child(1)");
		var ifr1_1=theDiv.find("iframe:nth-child(2)");
		var heights1=0;
		if(ifr1_0.length > 0) {
			heights1+=ifr1_0.height();
		}
		if(ifr1_1.length > 0) {
			heights1+=ifr1_1.height();
		}
		if (ifr1_0.length == 0 || heights1 === 0 || theDiv.css("display")  == "none") {
			...
			if (hasOptionalContent) {
				...
				console.log(params.vcrsPosition);
				if(params.vcrsPosition <= 1) {
					window.finishedVcrOptional1 = true;
				} else {
                    window.finishedVcrOptional2 = true;
...
function populateSponsoredPodcast(sponsoredPodcast, podcastChannel, language) {
	...
	var anchorUrl = getVcrAnchorUrl(sponsoredPodcast, "indexPage");
